...
Contributing
📚 Project Docs
README.md - complete project documentation, setup, modules, and deployment notes.
STARTUP_PITCH_20_SLIDES.md - ready-to-use 20-slide presentation layout.
🎯 Overview
...
```
   ...
   *.log
   ...
   config.php
   ```
Update connection.php for Production
...
Steps:
Deploy backend first (Hostinger/cPanel/Railway/VPS), example:
`https://your-backend-domain.com/MARK_16`
Open:
`vercel-ready/public/config.js`
Set:
`backendBaseUrl` to your backend URL
Deploy `vercel-ready` to Vercel
Open Vercel URL and verify buttons route to backend
...
Steps:
Upload full `MARK_16` folder to hosting (`public_html/MARK_16`)
Create MySQL DB and import `database/demo.sql` (or latest export)
Update `connection.php` DB credentials
Create `.env.local` from production example
Set cron for automation:
...
💻 Usage
For Users:
Register/Login at `signup.php` or `signin.php`
Fill food donation form
View donation history in profile
For Admins:
Register/Login at `admin/signup.php` or `admin/signin.php`
View donations in dashboard
Manage and assign donations
View analytics
For Delivery Persons:
Register/Login at `delivery/deliverysignup.php` or `delivery/deliverylogin.php`
